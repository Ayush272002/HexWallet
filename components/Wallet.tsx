...
import { deleteWallet, generateWalletFromMnemonic } from '@/lib/wallet';
...
const Wallet = () => {
  ...
  const togglePrivateKeyVisibility = (index: number) => {
    setVisiblePrivateKeys(
      visiblePrivateKeys.map((v, i) => (i === index ? !v : v))
    );
  };

  // const togglePhraseVisibility = (index: number) => {
  //   setVisiblePhrases(visiblePhrases.map((v, i) => (i === index ? !v : v)));
  // };
  ...
  const handleAddWallet = () => {
    if (!mnemonicWords) {
      toast.error('No mnemonic words found. Please generate a wallet first.');
      return;
    }
            ...
            {pathTypes.length === 0 && (
              ...
              >
                <div className="flex flex-col gap-2">
                  <h1 className="tracking-tighter text-4xl font-semibold md:text-5xl">
                    HexWallet supports multiple blockchains
                  </h1>
